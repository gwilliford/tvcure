...
#browser()
  while (convergence > eps & i < emmax) {
    ...
    # Update glm
      ...
      if (!is.null(offsetvar))
        update_cureg <- as.numeric(eval(parse(text = paste("glm","(", "w~Z[,-1] +offset(offsetvar)", ",family = quasibinomial(link='",link, "'", ")", ")", sep = "")))$coef)
    if (model == "ph") {
      if (!is.null(offsetvar)) {
        if (firthcox) {
          update_beta <- coxphf(survobj ~ X[,-1] + offset(offsetvar + log(w)),pl=F)$coefficients
        } else {
        ...
        if (firthcox) {
          ...
          tryCatch(
            coxit <- coxph(survobj ~ X[, -1] + offset(log(w)), subset = w != 0, method = "breslow"),
            error = function(e) e
          )
        }
      }
