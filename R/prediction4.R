...
predict.tvcure <- function(model, type = c("basesurv", "suncure", "spop", "uncureprob"),
  ...
  if (type == "suncure") {
    ...
    if (CI == T) {
      if (bw == F) {
        splot = splot + geom_ribbon(scf, mapping = aes(x = Time,
                                                       ymin = sclo, ymax = schi,
                                                       col = as.factor(num),
                                                       fill = as.factor(num),
  ...
  if (type == "spop") {
    ...
    if (bw == F) {
      splot = splot +
        geom_step(spf, mapping = aes(Time, spm, col = as.factor(num), linetype = as.factor(num)), size = 1) +
        labs(linetype = legendtitle, col = legendtitle, fill = legendtitle)
    } else {
      splot = splot +
        geom_step(spf, mapping = aes(Time, spm, linetype = as.factor(num)), size = 1) +
        labs(linetype = legendtitle)
    }
