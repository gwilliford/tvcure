...
set_cure = tvcure(Surv(start, stop, event) ~ lpchcap + recmidwt +recnowt +  bdymid,
                  cureform = ~ ltradedep_geomean + caprat,
                  data = peaceterm,
                  link = "probit",
                  nboot = 10,
                  brglm = T); summary(set_cure)
a = coxph(Surv(start, stop, event) ~ lpchcap + recmidwt +recnowt +  bdymid,
                  data = peaceterm); summary(a)
...
##### Quantities with CIs and newdata is only one row

j = tvpred(set_cure, type = "basesurv", CI = T)
k = tvpred(set_cure, type = "suncure", newX = newX, newZ = newZ, CI = T)
l = tvpred(set_cure, type = "spop", newX = newX, newZ = newZ, CI = T)
m = tvpred(set_cure, type = "uncureprob", newX = newX, newZ = newZ, CI = T)
