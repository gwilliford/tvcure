...
prediction2 <- function(model, type = c("basesurv", "spop", "suncure", "uncureprob"),
                        variable = NULL, values = NULL, insamp = F, CI = F, nsims = 1000,
  ...
  # if (insamp == F & (variable == NULL | values == NULL))
  #   stop("variable and values must be provided unless insamp = T")
...
# Simulate Data -----------------------------------------------------------
  ...
  if (!is.null(values) & !is.null(variable)) {
...
browser()
  ...
  geom_errorbar(aes(ymax = U, ymin = L))
    ...
    if (type == "suncure") {
      if (insamp == T) {
        scf <- data.frame(scm = as.vector(suncure), Time, num = 1)
        splot <- ggplot(scf, aes(Time, scm)) +
...
# uncure prob
# done - Export uncureprobsims
# done - Order s0 in a logical way
# done - are graphs behaving for suncure and spop
# Smooth the output of the lines
# done - Optimize the speed
# done - create error function if Z and X are not both specified when new
# Create functionality to just get baselines for population
# Make it only spit out an image, not just a summary
# Make it save graph as object
