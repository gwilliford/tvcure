smcuretest <- function (object, newX, newZ, model = c("ph", "aft"), ...)
{
    call <- match.call()
    ...
    n = nrow(s0)
    uncureprob = exp(object$b %*% t(newZ))/(1 + exp(object$b %*%
        t(newZ)))
    ...
    structure(list(call = call, newuncureprob = uncureprob, prediction = prd),
        class = "predictsmcure")
}
