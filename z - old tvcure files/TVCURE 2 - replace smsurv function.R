  ...
  #####Bootstrap standard errors
      if (Var) {
        ...
        while (i <= nboot) {
          ...
          if (ncol(Y)==2){
            bootfit <- emorig(bootdata[, 1], bootdata[, 2], bootX, bootZ, offsetvar, b, beta, model, link, emmax,eps)
          } else {
            bootfit <- emcount(bootdata[, 1], bootdata[, 2], bootdata[,3], bootX, bootZ, offsetvar, b, beta, model, link, emmax,eps)
          }
          b_boot[i, ] <- bootfit$b
