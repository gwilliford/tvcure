...
double sploglog_lnl(arma::vec theta, arma::mat y, arma::mat X, arma::mat Z) {
  ...
  int rx = X.n_cols - 1; //Todo: Drop -1 once X is fixed to not contain constant
  int rz = Z.n_cols - 1;
  ...
  // Define ???
  double p = theta[rx + rz + 2]; //Todo: +1 once X is fixed to not contain constant
  
  // Define hazard matrix 
  ...
  // ???
  vec rc = y.submat(0,0,y.n_rows-1,0); // right censored/censoring indicator? 
  vec ti = y.submat(0,1,y.n_rows-1,1); // failure time
  ...
  // Likelihood function for censored obs
  ...
  // Likelihood function for uncensored obs
  ...
  // Combined loglikelihood function
